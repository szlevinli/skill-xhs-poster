...
Runtime outputs are written to `xiaohongshu-data/`, including `today-pool.json`, `contents.json`, `publish-log.json`, and downloaded images under `xiaohongshu-data/images/{product_id}/`. Reference inputs for trend preparation live in `references/history-notes/`. Treat `references/` and `xiaohongshu-data/` as data directories, not source code.
